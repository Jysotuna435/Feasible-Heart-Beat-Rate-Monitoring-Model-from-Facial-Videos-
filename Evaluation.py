...
def evaluat_error(sp, act) -> object:
    ...
    # Calculate NMSE
    nmse = (sum(abs_r_x ** 2) / sum(abs_r ** 2))
    # Calculate MSE
    mse = sum((r[i] - x[i]) ** 2 for i in range(len(r))) / len(r)
    onenorm = sum(abs_r_x)
    twonorm = (sum(abs_r_x ** 2) ** 0.5)
    infinitynorm = max(abs_r_x)
    EVAL_ERR = [mep, smape, rmse, mase, mae, mse, nmse, onenorm, twonorm]
    return EVAL_ERR
...
def net_evaluation(sp, act):
    ...
    IoU = tp / (tp + fp + fn)
    MCC = ((tp * tn) - (fp * fn)) / math.sqrt((tp + fp) * (tp + fn) * (tn + fp) * (tn + fn))
    EVAL = [tp, tn, fp, fn, IoU, precision, sensitivity, Dice, Jaccard, accuracy, specificity, FPR, FNR, NPV, FDR, F1_score,
            MCC]
    return EVAL
